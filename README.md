...
for both providers**, not only Gemini. University of Cologne KI Connect
uses the tested `OpenAI GPT OSS 120b KI:Inferenz.nrw` model for the
Architect and Reviewer in the one-click setup — proven end to end, so
there is no live model list to choose from and setup never makes a
model-discovery request. The bundled `chroma_db/` knowledge
